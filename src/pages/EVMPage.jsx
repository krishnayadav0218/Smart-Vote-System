...
export default function EVMPage() {
  ...
  const [stationId, setStationId] = useState(localStorage.getItem("sv_station_id") || "");
  const saveStationId = (val) => {
    setStationId(val);
    localStorage.setItem("sv_station_id", val);
  };
  ...
  const checkMyReceipt = async () => {
    try {
      const { data } = await axios.get(`${API}/api/votes/check-receipt`, { params: { tx_hash: receipt.tx_hash } });
      setReceiptCheck(data.found ? "found" : "not_found");
    } catch {
      setReceiptCheck("unavailable");
    }
  };
    ...
    <div className="max-w-md mx-auto">
      <div className="bg-[#0b1529] border border-[#1e3050] rounded-2xl p-6 text-center">
        ...
        <div className="bg-[#111e35] border border-[#1e3050] rounded-xl p-4 text-left space-y-2 mb-4">
          {[["Voter ID", receipt.voter_id],["Name", receipt.voter_name],["Party", `${receipt.party_symbol || ""} ${receipt.party}`],["Constituency", receipt.constituency],...(receipt.station_id ? [["Station", receipt.station_id]] : []),["Tx Hash", receipt.tx_hash],["Receipt", receipt.receipt_hash],["Time", new Date(receipt.voted_at).toLocaleTimeString()]].map(([k,v])=>(
        ...
        <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-2 flex items-center gap-2 text-xs text-green-400 mb-3">
          ⛓️ Recorded in a hash-linked log · Cannot be quietly edited without breaking the chain
        </div>
        {usingLive && (
          <div className="mb-4">
            <button onClick={checkMyReceipt} className="text-xs bg-[#111e35] border border-[#1e3050] text-slate-300 hover:text-white px-3 py-2 rounded-lg w-full">
              🔍 Verify this receipt is in the chain
            </button>
            {receiptCheck === "found" && <p className="text-xs text-green-400 mt-2">✓ Confirmed on the server — your party choice stays private.</p>}
            {receiptCheck === "not_found" && <p className="text-xs text-red-400 mt-2">Not found — tell the election operator.</p>}
            {receiptCheck === "unavailable" && <p className="text-xs text-slate-500 mt-2">Could not reach the server to check.</p>}
          </div>
        )}
        <button onClick={() => window.print()} className="w-full mb-2 text-xs bg-[#111e35] border border-[#1e3050] text-slate-300 hover:text-white px-3 py-2 rounded-lg">
          🖨️ Print receipt
        </button>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 max-w-5xl">
      ...
      <div className="space-y-4">
        <div className="bg-[#0b1529] border border-[#1e3050] rounded-xl p-3 flex items-center gap-2">
          <span className="text-xs text-slate-400 whitespace-nowrap">🖥️ This device's booth ID:</span>
          <input
            value={stationId}
            onChange={e => saveStationId(e.target.value)}
            placeholder="e.g. BOOTH-1 (optional)"
            className="bg-[#111e35] border border-[#1e3050] text-white rounded-lg px-2 py-1 text-xs outline-none focus:border-blue-500 flex-1"
          />
        </div>
