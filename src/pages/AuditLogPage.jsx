...
const ACTION_COLOR = {
  ...
  LOGIN_BLOCKED_LOCKED: "text-red-400",
  ACCOUNT_LOCKED: "text-red-400",
  LOGOUT: "text-slate-400",
  LOGOUT_ALL_SESSIONS: "text-slate-400",
  ...
  VOTER_DELETED: "text-red-400",
  VOTER_PIN_RESET: "text-cyan-400",
  VOTERS_BULK_IMPORTED: "text-cyan-400",
  SETTINGS_UPDATED: "text-purple-400",
  OTP_ISSUED: "text-cyan-400",
  "2FA_ENABLED": "text-green-400",
  "2FA_DISABLED": "text-amber-400",
  "2FA_FAILED": "text-red-400",
  ELECTION_FINALIZED: "text-purple-400",
  HOUSEHOLDS_BULK_IMPORTED: "text-cyan-400",
};
...
export default function AuditLogPage() {
  ...
  const load = () => {
    setLoading(true);
    axios.get(`${API}/api/admin/audit-log`)
  ...
  const verify = () => {
    axios.get(`${API}/api/admin/audit-log/verify`)
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="flex gap-2">
          ...
          {live && <button onClick={verify} className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-lg">✅ Verify chain</button>}
      ...
      {verifyResult && (
        <div className={`text-xs rounded-lg px-3 py-2 mb-4 border ${verifyResult.valid ? "border-green-500/30 bg-green-500/10 text-green-300" : "border-red-500/30 bg-red-500/10 text-red-300"}`}>
          {verifyResult.valid
            ? `Audit chain verified — all ${verifyResult.entryCount} entries link correctly.`
            : `Problems found: ${verifyResult.problems.map(p => `#${p.id} (${p.issue})`).join(", ")}`}
        </div>
      )}
