...
- **Other admin logins are now real** — Settings → "Admin panel logins" (super_admin only) lets you create actual voter_manager / vote_tracker / election_officer accounts on the live backend. Before this, only "admin" existed on a real deployment — the other role names shown anywhere in the UI were offline-demo-only accounts that don't exist on a real server until you create them here.
- **...or seed those other logins purely from env vars** — set `SEED_VOTER_MANAGER_PASSWORD`, `SEED_VOTE_TRACKER_PASSWORD`, and/or `SEED_ELECTION_OFFICER_PASSWORD` on the **backend** service (not the frontend — the frontend has no login-related env vars, only `VITE_API_URL`) and they're created automatically on next restart, no UI clicking needed. See `.env.example` / `.env.render.example`.
