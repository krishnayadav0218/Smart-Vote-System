...
class AdminUser(Base):
    ...
    totp_secret = Column(String, nullable=True)       # set once 2FA setup begins
    totp_enabled = Column(Boolean, default=False)      # only true once the user confirms a code
    sessions_valid_after = Column(DateTime, default=datetime.utcnow)  # bump on password change/logout-all to invalidate old sessions


class AdminSession(Base):
    """
    Server-side session record backing an httpOnly cookie. Unlike a bare JWT,
    this can be revoked immediately (logout, password change) instead of
    just expiring client-side.
    """
    __tablename__ = "admin_sessions"
    token = Column(String, primary_key=True)  # opaque random string, stored in the cookie
    admin_username = Column(String, ForeignKey("admin_users.username"), nullable=False)
...
class ElectionSettings(Base):
    ...
    finalized = Column(Boolean, default=False)          # once true, results are certified and voting can't reopen
...
class VoteBlock(Base):
    ...
    station_id = Column(String, nullable=True)  # which polling device/booth cast this, for multi-booth audits
