...
def test_create_and_login_as_voter_manager(client):
    login(client)  # as super_admin
    r = client.post("/api/admin/users", json={
        "username": "manager1", "full_name": "Manager One",
        "password": "StrongPass1!", "role": "voter_manager",
    })
    assert r.status_code == 200

    client.post("/api/auth/logout")
    r = login(client, username="manager1", password="StrongPass1!")
    me = client.get("/api/auth/me").json()
    assert me["role"] == "voter_manager"

    # a voter_manager can add voters...
    r = client.post("/api/admin/voters", json={"id": "VM1", "full_name": "Test", "pin": "4826", "constituency": "Ward 1"})
    assert r.status_code == 200
    # ...but cannot change election settings (super_admin only)
    r = client.post("/api/admin/settings", json={"voting_open": False})
    assert r.status_code == 403
...
def test_invalid_role_rejected(client):
    login(client)
    r = client.post("/api/admin/users", json={
        "username": "bad", "full_name": "Bad Role", "password": "StrongPass1!", "role": "not_a_role",
    })
    assert r.status_code == 400
...
def test_weak_password_rejected_on_create(client):
    login(client)
    r = client.post("/api/admin/users", json={
        "username": "weakuser", "full_name": "Weak", "password": "password", "role": "vote_tracker",
    })
    assert r.status_code == 400


def test_only_super_admin_can_list_or_create_users(client):
    login(client)
    client.post("/api/admin/users", json={"username": "tracker1", "full_name": "Tracker", "password": "StrongPass1!", "role": "vote_tracker"})
    client.post("/api/auth/logout")
    login(client, username="tracker1", password="StrongPass1!")
    r = client.get("/api/admin/users")
    assert r.status_code == 403
    r = client.post("/api/admin/users", json={"username": "x", "full_name": "X", "password": "StrongPass1!", "role": "vote_tracker"})
    assert r.status_code == 403


def test_cannot_delete_self(client):
    login(client)
    r = client.delete("/api/admin/users/admin")
    assert r.status_code == 400


def test_reset_other_admin_password(client):
    login(client)
    client.post("/api/admin/users", json={"username": "officer1", "full_name": "Officer", "password": "StrongPass1!", "role": "election_officer"})
    r = client.post("/api/admin/users/officer1/reset-password", json={"new_password": "NewStrongPass9!"})
    assert r.status_code == 200

    client.post("/api/auth/logout")
    # old password no longer works
    r = client.post("/api/auth/login", json={"username": "officer1", "password": "StrongPass1!"})
    assert r.status_code == 401
    # new password works
    r = client.post("/api/auth/login", json={"username": "officer1", "password": "NewStrongPass9!"})
    assert r.status_code == 200


def test_delete_admin_user(client):
    login(client)
    client.post("/api/admin/users", json={"username": "temp1", "full_name": "Temp", "password": "StrongPass1!", "role": "vote_tracker"})
    r = client.delete("/api/admin/users/temp1")
    assert r.status_code == 200
    r = client.post("/api/auth/login", json={"username": "temp1", "password": "StrongPass1!"})
    assert r.status_code == 401


def test_optional_role_seeding_from_env(tmp_path, monkeypatch):
    db_file = tmp_path / "seed_test.db"
    monkeypatch.setenv("DATABASE_URL", f"sqlite:///{db_file}")
    monkeypatch.setenv("SEED_VOTER_MANAGER_PASSWORD", "VMPass123!")
    monkeypatch.setenv("SEED_VOTE_TRACKER_PASSWORD", "VTPass123!")
    ...
    with TestClient(main.app) as c:
        r = c.post("/api/auth/login", json={"username": "voter_manager", "password": "VMPass123!"})
        assert r.status_code == 200
        assert c.get("/api/auth/me").json()["role"] == "voter_manager"

        c.post("/api/auth/logout")
        r = c.post("/api/auth/login", json={"username": "vote_tracker", "password": "VTPass123!"})
        assert r.status_code == 200

        # election_officer wasn't given a password env var, so it shouldn't exist
        r = c.post("/api/auth/login", json={"username": "election_officer", "password": "anything"})
        assert r.status_code == 401
